...
router.post('/crearcancion',isAuth, async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({message:"No se pudo crear la canción, vuelva a intentarlo.", error});
        console.log(error)
    }
});
...
router.delete('/eliminarcancion/:id', isAuth, async (req, res) => {
    ...
    try {
        ...
        if(match) {
            ...
            const cancionEliminada = await cancionesModel.findByIdAndDelete(req.params.id)
            // console.log(cancionEliminada)
